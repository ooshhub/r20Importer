/* globals Scene, readTextFromFile */
...
export default class R20Importer {
    ...
    static async importCampaign(campaignObject) {
		...
		await Intro.drawLoadingPage(campaignObject);
	...
	static async importPages(pagesObj, isArray) { // rewrite to convert all data as Promises before creation???
		...
		await Promise.all(pagesArray.map(async (page) => {
			...
			let wallColours = [];
			page.paths
				.filter(w=>w.attributes.layer==='walls')
				.forEach(w=> {
					let paths = [];
					try{paths = JSON.parse(w.attributes.path)}catch(err){console.error(err)}
					let segmentCount = paths?.length-2
					if (!paths) wallColours.push(w.attributes.stroke);
					for (let i=0;i<segmentCount; i++) {
						wallColours.push(w.attributes.stroke);
					}
				});
