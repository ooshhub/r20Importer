/* globals Folder, CONFIG, ui, Actor, */
...
export default class Helpers {
    ...
    static async getAverageFromArray (inputArr, returnIndex=0) { // index 0 returns highest, 1 returns second highest etc.
        ...
        let sorted = Object.values(tally).sort((a,b)=>b-a);
        //let max2 = Object.values(tally).sort((a,b)=>b-a)[1];
        for (let i in tally) {
            if (tally[i] === sorted[returnIndex]) return i;
        }
        return null;
    }
    ...
    static async grabLightSource(token, system, offsets, nocheck=false) {
            ...
            )
        {
            ...
            //let brightRadiusL, dimRadiusL, lightAngleL;
            if (system === 'updated') {
                brightRadius = (ta.emits_bright_light) ? parseInt(ta.bright_light_distance, 10) : 0;
                dimRadius = (ta.emits_low_light) ? parseInt(ta.low_light_distance, 10) : 0;
                lightAngle = parseInt(ta.directional_bright_light_total, 10)||0;
                if (nocheck && (ta.has_bright_light_vision || ta.has_low_light_vision)) {
                    brightVision = parseInt(ta.bright_light_vision, 10);
                    dimVision = (ta.has_night_vision) ? parseInt(ta.night_vision_distance, 10) : 0;
                ...
                if (nocheck || ta.light_hassight) {
                    brightVision = (ta.light_otherplayers) ? 0 : brightRadius;
                    dimVision = (ta.light_otherplayers) ? 0 : dimRadius;
            ...
            lightAngle = parseInt(ta.directional_bright_light_total, 10)||0;
            brightRadiusL = (!(ta.light_dimradius) || isNaN(ta.light_dimradius)) ? parseInt(ta.light_radius, 10)||0 : Math.max(0, ( Math.min(parseInt(ta.light_dimradius, 10), parseInt(ta.light_radius, 10))));
            dimRadiusL = (parseInt(ta.light_radius, 10) <= brightRadius) ? 0 : parseInt(ta.light_radius, 10)||0;
            lightAngleL = parseInt(ta.light_angle, 10)||0;*/
            ...
            let lightData = {
                ...
                dim: (system === 'updated' || ta.light_otherplayers) ? dimRadius : 0,
                bright: (system === 'updated' || ta.light_otherplayers) ? brightRadius : 0,
