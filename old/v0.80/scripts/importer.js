/* globals Scene, readTextFromFile, JournalEntry, Actor, Macro, RollTable, CONFIG */
...
export default class R20Importer {
	...
	static counter = {
		...
		cleanups: 0,
		_total: 0,
		_weights: {scenes: 10, folders: 10, actors: 2, journals: 1, rolltables: 1, macros: 1, cleanups: 5},
	...
	static async importPages(pagesObj, isArray) { // rewrite to convert all data as Promises before creation???
		...
		await Promise.all(pagesArray.map(async (page) => {
			...
			let doorColor = await H.getAverageFromArray(wallColours, true);
					...
					if (!wall.attributes.path.match(/[QC]/g)) {
						...
						for (let i=1; i<p.length; i++) {
							...
							outputWalls.push({
								...
								move: 1,
								sense: 1,
								door: (wall.attributes.stroke === doorColor) ? 1 : 0,
								ds: 0,
								flags: {},
								locked: true,
							});
            ...
            for (let layer in page.graphics) { // loop through layer by layer
				...
				await Promise.all(graphics.map(async (token, i) => {
					...
					if (layer === 'other') {  // Process tokens on Dynamic Lighting layer ==> Lighting layer
						...
						await H.grabLightSource(token, this.importConfig.currentPageLighting, this.importConfig.offsets, true)
							.then(lightCalcs => {
								newData = {
			...
			await Scene.create(sceneData).then(fScene => {
